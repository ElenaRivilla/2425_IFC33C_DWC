...
function generarRecibo(recibo) {
    ...
    for (let i = 0; i < recibo.length; i++) {
        let producto = recibo;
        let subtotal = calcularSubtotal(producto[i]);
        total += subtotal;
        
        console.log(producto[i].nombre.padEnd(21), producto[i].cantidad.toString().padEnd(11), producto[i].precio.toString().padEnd(15), subtotal);
    }
    calcularSubtotal(producto1);
...
function generarTodosLosRecibos(recibos) {
    for (let i = 0; i < recibos.length; i++) {
        generarRecibo(recibos[i]);
    }
}

generarTodosLosRecibos(recibos);